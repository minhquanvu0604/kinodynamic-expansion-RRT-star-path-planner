...
TEST(AckermanReachGoals, TwoGoals) {
    ...
    while (!OK){
        ...
        std::cout << "PERCENT " << progress.at(0) << std::endl;

        if(progress.front() == 100){ // we check progess, at 100 mission is acomplished
            //mission accomplished

            // DEBUG
            std::cout << "u - STATUS: " << progress.at(0) << std::endl;
    ...
    double distance = pow ( pow(odo.position.x-goals.back().x,2) + pow(odo.position.y-goals.back().y,2),0.5);
