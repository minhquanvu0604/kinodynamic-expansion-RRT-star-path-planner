...
TEST(QuadcopterReachGoal, SingleGoal) {
    ...
    while (!OK){
        ...
        if(status == pfms::PlatformStatus::IDLE){
            ...
            std::cout << "The Status is IDLE" << std::endl;
            OK=true;
        }
    ...
    double distance = pow ( pow(odo.position.x-pt.x,2) + pow(odo.position.y-pt.y,2),0.5);
