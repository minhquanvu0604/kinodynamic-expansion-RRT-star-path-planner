...
TEST(OutGoal,OutGoal1){
    ...
    path += "/test/bag/";
    std::string file = path + "ingoal1.bag";
    ...
    auto closestCones = process.getClosestPoints(cones, ackermanPosition);
    closestCones.push_back(ackermanPosition);

    auto hull = process.convexHull(closestCones);

    Point currentGoal(47.124722,22.876062);
    bool inPath = process.pointInConvexPolygon(hull, currentGoal);


    EXPECT_FALSE(inPath);
}

TEST(OutGoal,OutGoal2){
    ...
    // Now we have the path, the images for our testing are stored in a subfolder /test/samples
    path += "/test/bag/";
    std::string file = path + "ingoal2.bag";
    ...
    Point currentGoal(57.294720,-2.195305);
    bool inPath = process.pointInConvexPolygon(hull, currentGoal);
...
TEST(OutGoal,OutGoal3){
    ...
    // Now we have the path, the images for our testing are stored in a subfolder /test/samples
    path += "/test/bag/";
    std::string file = path + "ingoal3.bag";
    ...
    Point currentGoal(85.357544,-40.630745);
    bool inPath = process.pointInConvexPolygon(hull, currentGoal);
