...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
     ...
     */
    std_msgs::String msg; // This is our message type

    // To populate the message type, we need to assign the data portion of the message with a string
    std::stringstream ss;
    ss << "hello world " << count;
    msg.data = ss.str();

    ROS_INFO_STREAM("We are sending " <<  msg.data);
