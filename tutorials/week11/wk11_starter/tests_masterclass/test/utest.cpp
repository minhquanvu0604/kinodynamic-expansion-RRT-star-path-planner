...
TEST(LaserProcessing,DetectClosest){
    ...
    //! Below command allows to find the folder belonging to a package, you need to have
    //! correct package name
    std::string path = ros::package::getPath("week11_laser");
    ...
    //! We assign both laser and odometry to nullptr (as they are pointers)
    //! And then we can check we have the data 
    ...
    for(rosbag::MessageInstance const m: rosbag::View(bag))
    {
      ...
      if(m.getTopic() == "/drone/laser/scan"){
    ...
    //If we have it in local coordinates we can check the value here
