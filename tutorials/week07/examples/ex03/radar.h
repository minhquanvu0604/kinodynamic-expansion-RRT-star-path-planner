...
private:
    ...
    std::mutex mtx_; // USED TO PROTECT data_ AND ready_
    std::condition_variable cv_; // SIGNAL data_ UPDATED, ready_ TURNING TO true


    std::mutex mtxStart_; // USED TO PROTECT running_
    std::condition_variable cvStart_; // SIGNAL running_ CHANGED TO true

    std::vector<std::thread> threads_; // We add threads onto a vector here to be able to terminate then in estructor
    // ACTUALLY 1 THREAD PER RADAR ONLY
